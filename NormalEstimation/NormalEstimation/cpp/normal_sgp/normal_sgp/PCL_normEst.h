...
public:
	 ...
	 * @param normals
	 */
	PCL_Normal_Estimator(pcl::PointCloud<pcl::PointXYZ>::Ptr points, pcl::PointCloud<pcl::Normal>::Ptr normals):
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr& point_cloud(){return pts;}
	pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud()const{return pts;}
	pcl::PointCloud<pcl::Normal>::Ptr& normal_cloud(){return nls;}
	pcl::PointCloud<pcl::Normal>::Ptr normal_cloud() const{return nls;}
...
private:
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr pts;/*!< Point cloud*/
	pcl::PointCloud<pcl::Normal>::Ptr nls;/*!< Normal cloud*/
	...
	inline void assign_points_to_cubes(std::vector<int> &pointIdxRadiusSearch, std::vector<int> cubes[],
			float radius, pcl::PointXYZ &refPoint, pcl::PointXYZ points[],
		...
		for(unsigned int i=0; i<pointIdxRadiusSearch.size(); i++){
			...
			pcl::PointXYZ refPoint2 = refPoint;
	 ...
	 */
	inline void find_a_triplet(pcl::KdTreeFLANN<pcl::PointXYZ> &tree, float radius1, float radius2, Eigen::Vector3i &triplet,
			pcl::PointXYZ &pt){
	...
	inline void generate_list_of_triplets(std::vector<Eigen::Vector3i> &triplets, 
			...
			float radius2,pcl::KdTreeFLANN<pcl::PointXYZ> &tree,
			pcl::PointXYZ & pt,
...
#pragma omp parallel for schedule(guided)
		...
		for(int n=0; n<(int)pts->size(); n++){
			...
			pcl::PointXYZ *points = new pcl::PointXYZ[points_size];
...
#pragma omp parallel for schedule(guided)
		...
		for(int n=0; n<(int) pts->size(); n++){
			...
			pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_neighbors (new pcl::PointCloud<pcl::PointXYZ>);
			cloud_neighbors->resize(pointIdxRadiusSearch.size());
			for(unsigned int i=0; i<pointIdxRadiusSearch.size(); i++){
				cloud_neighbors->points[i] = pts->points[pointIdxRadiusSearch[i]];
			}
			pcl::KdTreeFLANN<pcl::PointXYZ> tree_neighbors;
			tree_neighbors.setInputCloud(cloud_neighbors);

			pcl::PointXYZ *points = new pcl::PointXYZ[neighbor_number];
