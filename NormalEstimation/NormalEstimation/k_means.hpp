...
template<typename T> double calculate_means(vector<int>& belongs_to, vmean& means, const vector<vector<T>>& points) {
	...
	forn(i, k) for(int ii=0;ii<means[i].size();ii++){
		double &coord = means[i].at(ii);
		coord=0.0;
	}
    forn(i, n) {
    ...
    forn(i, k) {
        if(point_count[i] == 0) {
			...
			//for(double& coord : means[i]) coord /= point_count[i];
			for(int ii=0;ii<means[i].size();ii++){
				double &coord = means[i].at(ii);
				coord /= point_count[i];
			}
		}
...
template<typename T> void initialize(vector<int>& belongs_to, vmean& means, const vector<vector<T>>& points, int k) {
	...
	for (int i=0;i<belongs_to.size();i++)
	{
		int &x=belongs_to.at(i);
		x=partitions(gen);
	}
